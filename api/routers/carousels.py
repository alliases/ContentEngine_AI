...
from api.schemas.content import CarouselDetailResponse, CarouselResponse, SlideResponse
...
async def get_carousel(
    ...
    stmt_carousel = select(Carousel).where(
        Carousel.id == carousel_id, Carousel.tenant_id == uuid.UUID(tenant_id)
    )
    ...
    if not carousel:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Carousel not found"
        )

    stmt_slides = (
        select(Slide).where(Slide.carousel_id == carousel.id).order_by(Slide.position)
    )
